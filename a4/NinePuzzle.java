...
public class NinePuzzle{
	...
	*/
	public static boolean SolveNinePuzzle(int[][] B){
		...
		//printBoard(getBoardFromIndex(365));
		
		//using an adjacency list, construct a
		//graph of all boards
		int[][] adjList = new int[NUM_BOARDS][4];
		for(int x = 0; x < NUM_BOARDS; x++){
			adjList[x] = createAdjList(x);
		}
		
		//index of the starting board
		int v = getIndexFromBoard(B);
		
		//queue for bfs
		LinkedList<Integer> q = new LinkedList<Integer>();
		
		//array to keep track of the path of the traversal
		int[] cameFrom = new int[NUM_BOARDS];
		
		int temp = v;
		cameFrom[v] = v;
		
		//while not solved (0 is the index for the solved board)
		while(temp != 0){
			
			//if it's not the forst iteration and the queue is empty,
			//then there is no path, so return false
			if(temp != v && q.size() == 0){
				return false;
			}
			
			//for all values in the adjacency list
			for(int x = 0; x < 3; x++){
				
				//if there is a valid board that hasn't been visited yet
				if(adjList[temp][x] != -1){
					if(cameFrom[adjList[temp][x]] == 0){
						
						//add the index to the queue
						q.add(adjList[temp][x]);
						
						//put the current index (temp) into the 
						//cameFrom list to keep track of the path
						cameFrom[adjList[temp][x]] = temp;
					}
				}
			}
			
			//take off of the top of the queue
			temp = q.poll();
		}
		
		//follow the path from solved state to initial state
		//while putting each index into the list
		...
		//now go the opposite direction (initial state to solved state)
		//and print each board
		while(x != 0){
			printBoard(getBoardFromIndex(path[--x]));
		}	
		
		//return true since there is a solution
		return true;
	}
	
	private static int[] createAdjList(int index){
		
		int[][] board = getBoardFromIndex(index);
		
		//for any board, there are a maximum of 4
		//possible moves. If there are less than 4,
		//the remaining entries will be set to -1
		int[] result = new int[4];
		
		//find the location of the 0 in the board
		int x = 0;
		int y = 0;
		for(int a = 0; a < 3; a++){
			for(int b = 0; b < 3; b++){
		...
		//z keeps track of the number of list entries
		int z = 0;
		
		//one move will be performed on temp, changing it
		//into a board that is connected to the current board
		int[][] temp = board;
		
		//different moves based on the location of the 0 on the board
		if(x == 0){
			temp[x][y] = temp[x + 1][y];
			temp[x + 1][y] = 0;
			result[z++] = getIndexFromBoard(temp);
			temp = getBoardFromIndex(index);
		} else if(x == 2){
			temp[x][y] = temp[x - 1][y];
			temp[x - 1][y] = 0;
			result[z++] = getIndexFromBoard(temp);
		...
		//remaining entries are changed to -1
		while(z < 4){
			result[z++] = -1;
		}
